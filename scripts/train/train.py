...
def build_emb_matrix():
    emb_mat = np.zeros((len(vocab)+1, args.glove_dim))
...
if TRAIN_MODEL == 'trans_lstm':
    model = Infotab_model(MODEL_NAME)
else:
...
for ep in range(25):
    ...
    dev_acc, dev_gold, dev_pred = test(model ,val_dataloader)
...
a1_acc, _, _ = test(model ,a1_dataloader)
print(a1_acc)
a2_acc, _, _ = test(model ,a2_dataloader)
print(a2_acc)
a3_acc, _, _ = test(model ,a3_dataloader)
print(a3_acc)
